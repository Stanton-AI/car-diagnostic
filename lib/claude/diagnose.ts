...
1. 증상을 보고 가장 의심되는 계통(엔진/제동/전기/냉각/연료/변속/서스펜션/배기 등)을 특정
2. 해당 계통 내에서 가능한 하위 원인군을 나열
3. 기존 Q&A가 있다면 → 어떤 하위 원인군으로 좁혀졌는지 파악
4. 아직 원인이 좁혀지지 않았다면 → 하위 원인군을 구분할 수 있는 질문 1개 생성
5. 충분히 좁혀졌다면 (confidence >= 75) → sufficient: true

[질문 원칙]
- 각 질문은 "어떤 가설을 검증하는가"가 명확해야 함
- 계통 → 하위계통 → 특정 원인 순으로 좁혀가는 질문
