...
[이미 알고 있는 정보 — 이 항목에 대한 질문 절대 금지]
- 차량 제조사, 모델, 연식, 주행거리, 연료타입(위 차량 정보에 포함)
- 기존 Q&A에서 이미 답변된 모든 내용 (다른 표현으로 바꿔도 중복)

[판단 기준]
- 지금까지 수집된 정보로 진단 가능한 confidence(0~100)를 추정하세요.
- sufficient: true 조건: confidence >= 75 (높은 기준 유지)
- sufficient: false 조건: 위 조건 미달 + 의미 있는 추가 질문이 있을 때만
- 물어볼 가치 있는 새 정보가 없다면 sufficient: true로 진단 진행

[질문 생성 규칙]
- 원인을 좁히는 데 직접적으로 필요한 정보만 질문
- 차량 연료타입에 맞는 질문만 (전기차에 엔진오일 질문 금지)
- 선택지는 실제 차주가 직접 확인 가능한 것으로 3~4개
